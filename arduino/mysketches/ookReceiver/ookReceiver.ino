...
// These are the 3rd and 4th bytes of the BBSB packet, which is
// all we need to distinguish which button has been pressed
const byte BUTTON1[] = {85, 85};
const byte BUTTON2[] = {77, 85};
const byte BUTTON3[] = {213, 84};
const byte BUTTON4[] = {205, 84};
...
static void runPulseDecoders (DecoderInfo* pdi, volatile word& pulse) {
    ...
    if (p != 0) { 
      ...
      // Flip the state bit from 0 to 1, or 1 to 0
      state = (state + 1) % 2;
      Serial.println(totalTime + 1);
      //Serial.print(" ");
      //Serial.println(state);
      //Serial.print(total);
      //Serial.print(" ");
...
void loop () {
    ...
    if (sendTimer.poll(100) && packetFill > 0) {
        ...
        /*
           ...
           rf12_sendNow(0, packetBuffer2, packetFill2);
           rf12_sendWait(1);
           sendPacket = 0;*/
#if PIN_868
        rf12_init_OOK();
