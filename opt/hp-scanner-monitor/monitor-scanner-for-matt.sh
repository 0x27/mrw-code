...
do
   ...
   # For now, don't write to the log when we're trying to connect to the printer
                        ...
                        if [[ -z "$xml" ]]; then
                           ...
                           if [[ -z "$xml" ]]; then
                              ...
                              do
                                 ...
                                 echo "A scanned image is attached" | mutt -s "Scanned Image" -a /tmp/image.jpg -- matthew1001@gmail.com 2>>/tmp/log.txt
                                 rm /tmp/image.jpg
