...
sub connect
{
	...
	my $clientName = shift || die "No client name specified";
        ...
        #$self->{_mqtt} = new WebSphere::MQTT::Client(
   ...
   eval {
	  ...
	  if($self->{_debug}){
	        ...
	        #&upub_publish ("local/rfm12/nodes/" . $nodeNumber . "/lastreceived","y",$now_string);
                $self->{_mqtt}->publish("local/rfm12/raw/test/" . $nodeNumber, 1, 0, $value);
                ...
                $self->{_mqtt}->publish("local/rfm12/nodes/test/" . $nodeNumber . "/lastreceived", 1, 1, $now_string);
                ...
                if ($nodeNumber eq "02" || $nodeNumber eq "03") {
                  ...
                  $self->{_mqtt}->publish("local/rfm12/nodes/test/" . $nodeNumber . "/temp", 1, 1, $temperature);
                  $self->{_mqtt}->publish("local/rfm12/nodes/test/" . $nodeNumber . "/humidity", 1, 1, $humidity);
                  $self->{_mqtt}->publish("local/rfm12/nodes/test/" . $nodeNumber . "/voltage", 1, 1, ($voltage/10));

                  # Update the database (if the last update wasn't very recently)
                     my $query_handle = $self->{_db}->prepare_cached('INSERT INTO testtemperature (`temperature`) VALUES (?)');
                     $query_handle->execute($temperature);
                     $query_handle->finish();
                     my $query_handle2 = $self->{_db}->prepare_cached('INSERT INTO testhumidity (`humidity`) VALUES (?)');
                     $query_handle2->execute($humidity);
                     $query_handle2->finish();
                     my $query_handle3 = $self->{_db}->prepare_cached('INSERT INTO testwirelessnodes (`nodeid`, `voltage`) VALUES (?, ?)');
       ...
       if (!Encode::is_utf8($value)) { 
          print STDERR "(The other data we've received is not UTF8 so it will be discarded)\n";
       }
       if ($value !~ /[^[:ascii:]]/) {
          print STDERR "(The other data we've received is not ASCII so it will be discarded)\n";
       }
