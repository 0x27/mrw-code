...
protected:
    ...
    virtual char decode (word width, long timestamp, byte bitArray[], long timestamps[], byte * bitIndex) = 0;
...
public:
    ...
    bool nextPulse (word width, long timestamp, byte bitArray[], long timestamps[], byte * bitIndex) {
        if (state != DONE)
            switch (decode(width, timestamp, bitArray, timestamps, bitIndex)) {
...
public:
    ...
    virtual char decode (word width, long timestamp, byte bitArray[], long timestamps[], byte * bitIndex) {
            ...
            switch (state) {
                ...
                case T2:
                    if (w) {
                        gotBit(0);
                        bitArray[(*bitIndex)] = 0;
                        timestamps[(*bitIndex)] = timestamp;
                        (*bitIndex)++;
                        if (*bitIndex >= BITARRAYSIZE) {
                          // We have to wrap the index counter if we hit
                          // the upper limit of what we can store
                          *bitIndex = 0;
                        }
                        //Serial.print("BIT ");
